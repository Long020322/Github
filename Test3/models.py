...
class CP(nn.Module):#单个CP
    ...
    def forward(self, i_input, j_input, k_input ):
        ...
        # 计算 cp_embed
        cp_embed = i_embeds * j_embeds * k_embeds  # [128, latent_dim]

        xijk = torch.einsum('nd, nd, nd -> n', i_embeds, j_embeds, k_embeds)

        return cp_embed, xijk
    
class AggregationModel(nn.Module):
    def __init__(self,latent_dim):
        super().__init__()
        self.fc1 = nn.Linear(latent_dim * 4, 256)    # 增加宽度
        self.fc2 = nn.Linear(256, 128)
        self.fc3 = nn.Linear(128, 64)
        self.fc4 = nn.Linear(64, 32)
        self.output_layer = nn.Linear(32, 1)

        self.activation = nn.LeakyReLU(0.1)  # 使用 Leaky ReLU
        self.batch_norm1 = nn.LayerNorm(256)
        self.batch_norm2 = nn.LayerNorm(128)
        self.batch_norm3 = nn.LayerNorm(64)
        self.batch_norm4 = nn.LayerNorm(32)

    def forward(self, output1, output2, output3, output4):
        combined_output = torch.cat([output1, output2, output3, output4], dim=-1)

        x = self.activation(self.batch_norm1(self.fc1(combined_output)))
        x = self.activation(self.batch_norm2(self.fc2(x)))
        x = self.activation(self.batch_norm3(self.fc3(x)))
        x = self.activation(self.batch_norm4(self.fc4(x)))

        xijk = self.output_layer(x)
        xijk = xijk.squeeze(1)
        return xijk
